...
app.get("/", (req, res) => {
  ...
  connection.query(
    ...
    function (error, postList, fields) {
      if (error) throw error;
...
app.post("/processCreate/:boardName", (req, res) => {
  ...
  connection.query(
    `INSERT INTO post VALUES (NULL, '${req.body.title}', '${req.body.description}', "익명",'${now}', '${req.params.boardName}')`,
...
app.get("/board/:boardName/:page", (req, res, next) => {
  connection.query(
    `SELECT COUNT(id) FROM post WHERE board='${req.params.boardName}'`,
...
app.get("/board/:boardName/:page", (req, res) => {
  ...
  connection.query(
    `SELECT * FROM post WHERE board='${
      req.params.boardName
    }' ORDER BY id DESC LIMIT ${(req.params.page - 1) * 20}, 20`,
    function (error, postList, fields) {
      if (error) throw error;
...
app.post("/processUpdate/:boardName/:id", (req, res) => {
  let query = `UPDATE post SET title=?, description='${req.body.description}' WHERE id=${req.params.id}`;
  connection.query(query, [req.body.title], function (error, post, fields) {
    res.redirect(`/board/${req.params.boardName}/1`);
  });
});
